...
export class AddUsersComponent implements OnInit {
  ...
  ngOnInit() {
    this.currentUsers.push(...this.note().sharedUsers!, this.note().ownerUsername)
  }
  ...
  handleEnter(event: any) {
    if (this.inputVal && !((event.target as HTMLElement).classList[0] === 'add-btn')) {
      this.addUser()
    }
  }
  ...
  addUser() {
    if (this.inputVal) {
      ...
      this.http.post(`${backendBaseURL}/username-exists`, this.inputVal).subscribe({
        next: (res: any) => {
          if (res) {
  ...
  sendAddUserReq() {
    console.log(this.usernamesToAdd)
    this.notesService.addUsersToNote(this.usernamesToAdd, this.note().id).subscribe({
      next: (res: any) => {
        console.log(res)
