...
export class SharedNotesPageComponent implements OnInit {
  ...
  fetchSharedNotes() {
    this.http
      .get(
        `${backendBaseURL}/shared-notes?user=${localStorage.getItem(
          'notepad-username'
        )}`
      )
      .subscribe({
  ...
  populateSharedUsersList() {
    this.sharedNotes?.forEach((note: SharedNote) => {
      ...
      if (note.sharedUsers.includes(note.ownerUsername)) {
        note.sharedUsers.splice(
          note.sharedUsers.indexOf(note.ownerUsername),
          1
        );
      }
    });
  ...
  createNewSharedNote() {
    ...
    this.http
      .post(`${backendBaseURL}/create-shared-note`, reqBody, {
        headers: headers,
      })
      .subscribe({
        next: (data: any) => {
  ...
  viewSharedNote(note: SharedNote) {
    if (!note.removeUsers) this.router.navigateByUrl('view-shared/' + note.id);
  }
  ...
  renameNote(input: string, note: SharedNote, usernameMakingChange: string) {
    if (!input) return;
    this.notesService.renameNote(note.id, input, usernameMakingChange).subscribe({
      next: () => {
        window.location.reload();
      },
      error: (e) => {
  ...
  sendRemoveUsersReq(note: SharedNote) {
    this.notesService
      .removeMultipleUsersFromNoteByUsername(note.id, note.usersToRemove!)
      .subscribe({
        next: (data: any) => {
          console.log(data);
          window.location.reload();
  ...
  showPreview(note: SharedNote) {
    if (note.text && !note.removeUsers) note.isViewingPreview = true;
  }

  hidePreview(note: SharedNote) {
    note.isViewingPreview = false;
  }
